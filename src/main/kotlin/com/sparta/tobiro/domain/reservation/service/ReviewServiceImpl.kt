...
class ReviewServiceImpl(
    ...
    override fun createReview(request: CreateReviewRequest): ReviewResponse {
        ...
        if (memberPrincipal.id != findReservation.member.id) {throw InvalidCredentialException()}

        reviewRepository.findReviewByReservation(findReservation)?.let {
            if (it.reservation.member.id == memberPrincipal.id) throw IllegalStateException("이미 후기가 존재합니다. 존재하는 후기의 수정, 삭제만 가능합니다.")
        }
    ...
    override fun updateReview(reviewId: Long, request: UpdateReviewRequest): ReviewResponse {
        ...
        if (review.reservation.member.id != memberPrincipal.id
                || memberPrincipal.authorities.first().authority != "ROLE_MEMBER")
